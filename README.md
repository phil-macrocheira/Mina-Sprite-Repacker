...
8) If you made a new mod, the structure is the same as `unpak`, but only include the `.anb.yc` sprites that you are modding. Also include the `_my_sprites` folder, which should also include only sprites you are modding.
9) Modify sprites
10) Run `mina-sr -r` to repack all sprites in the `_my_sprites` folder into the files in the `data` folder.
